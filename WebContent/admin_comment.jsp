			...
			<div class="sidebar-scroll">
				...
				<ul class="nav">
					...
					<li><a
						...
						class="active"><i class="lnr lnr-heart"></i><span>评论管理</span></a></li>
					<li><a
						href="<%=basePath%>user?option=adminFindUser&currPage=1"
						class=""><i class="lnr lnr lnr-user"></i><span>用户管理</span></a></li>
	...
	<div
		...
		<div class="table-responsive" style="width: 90%; margin: 0px auto;">
			<table class="table table-striped table-bordered table-hover">
				...
				<c:forEach items="${page.list}" var="list">
					<tr>
						<td style="vertical-align: middle;"><span>${list.comment.yxsj_comment_id}</span></td>
						...
						<td style="vertical-align: middle;"><span>${list.comment.comment_content}</span></td>
						<td style="vertical-align: middle;"><span>${list.comment.comment_gmt_create}</span></td>
						<td style="vertical-align: middle;"><span>
								<button class="btn btn-danger"
									style="margin: 10px; float: right;"
									onclick="window.location='<%=basePath%>comment?option=adminDeleteComment&yxsj_comment_id=${list.comment.yxsj_comment_id}'">删除</button>
						</span></td>
					</tr>
				</c:forEach>
			</table>
		...
		<div style="margin: 0 auto; width: 400px; text-align: center;">
			...
			<button class="btn btn-primary"
				onclick="window.location='<%=basePath%>comment?option=adminFindComment&currPage=1'">首页</button>
			...
			<c:choose>
				...
				<c:otherwise>
					<button class="btn btn-primary"
						onclick="window.location='<%=basePath%>comment?option=adminFindComment&currPage=${page.currPage-1}'">上一页</button>
				</c:otherwise>
			</c:choose>
			...
			<c:choose>
				...
				<c:otherwise>
					<button class="btn btn-primary"
						onclick="window.location='<%=basePath%>comment?option=adminFindComment&currPage=${page.currPage+1}'">下一页</button>
				</c:otherwise>
			</c:choose>
			...
			<button class="btn btn-primary"
				onclick="window.location='<%=basePath%>comment?option=adminFindComment&currPage=${page.totalPage}'">尾页</button>
		</div>
		<!-------------------------------------------------------------------------------------------------------------------------------------------------------->
